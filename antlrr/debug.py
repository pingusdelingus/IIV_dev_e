...
class TPTPModalExtractor(TPTPListener):
    ...
    def term_is_variable(self, term_ctx) -> bool:
        ...
        """Check if a term is, or contains, a variable."""
        # Path 1: Direct variable child of tff_term
        if hasattr(term_ctx, "variable") and callable(term_ctx.variable) and term_ctx.variable() is not None:
            return True
        
        # Path 2: Nested variable (term -> logic_formula -> ... -> plain_atomic -> variable)
            ...
            if f and hasattr(f, "tff_unitary_formula") and callable(f.tff_unitary_formula):
                ...
                if u and hasattr(u, "tff_atomic_formula"):
                    ...
                    if a and hasattr(a, "tff_plain_atomic"):
                        ...
                        # Check if the plain atomic is a variable
                        if p and hasattr(p, "variable") and p.variable():
                            return True
        return False
...
#                    self.accessible_worlds[from_world].append(to_world)
    ...
    def get_term_text(self, term_ctx):
        ...
        # Path 1: Direct variable
        if hasattr(term_ctx, 'variable') and callable(term_ctx.variable):
            var_ctx = term_ctx.variable()
            if var_ctx:
                return var_ctx.getText()
        
        # Path 2: Nested constant or variable
        if hasattr(term_ctx, 'tff_logic_formula') and callable(term_ctx.tff_logic_formula):
            f = term_ctx.tff_logic_formula()
            if f and hasattr(f, "tff_unitary_formula") and callable(f.tff_unitary_formula):
                u = f.tff_unitary_formula()
                if u and hasattr(u, "tff_atomic_formula"):
                    ...
                    if a and hasattr(a, "tff_plain_atomic"):
                        ...
                        if p:
                            # Check for constant
                            if hasattr(p, "constant") and p.constant():
                                return p.constant().getText()
                            # Check for variable
                            if hasattr(p, "variable") and p.variable():
                                return p.variable().getText()
    ...
    def extract_typed_variables(self, var_list_ctx):
        ...
        def get_var_info(var_ctx):
            ...
            if var_ctx and var_ctx.tff_typed_variable():
                typed_var = var_ctx.tff_typed_variable()
                if typed_var.variable() and typed_var.tff_atomic_type():
                    var_name = typed_var.variable().getText()
                    var_type = typed_var.tff_atomic_type().getText()
        ...
        if hasattr(var_list_ctx, 'tff_variable') and callable(var_list_ctx.tff_variable):
            first_var = var_list_ctx.tff_variable()
            # Handle list vs single
            if isinstance(first_var, list):
                info = get_var_info(first_var[0]) # Should only be one
            else:
                info = get_var_info(first_var)
            if info:
                variables.append(info)


        # Handle subsequent variables (comma_tff_variable)
        if hasattr(var_list_ctx, 'comma_tff_variable') and callable(var_list_ctx.comma_tff_variable):
            comma_vars = var_list_ctx.comma_tff_variable()
            if comma_vars:
                if isinstance(comma_vars, list):
                    for cv in comma_vars:
                        if cv.tff_variable():
                            info = get_var_info(cv.tff_variable())
                            if info:
                                variables.append(info)
                else: # It's a single element, not a list
                    if comma_vars.tff_variable():
                        info = get_var_info(comma_vars.tff_variable())
                        if info:
                            variables.append(info)
