...
class TPTPModalExtractor(TPTPListener):
    ...
    def get_unitary_term_text(self, u_ctx):
        """(NEW) Extract text from a tff_unitary_term context."""
        
        # Path 1: It's a direct variable
        if hasattr(u_ctx, 'variable') and callable(u_ctx.variable) and u_ctx.variable():
            return u_ctx.variable().getText()

        # Path 2: It's a nested atomic
        if hasattr(u_ctx, 'tff_atomic_formula') and callable(u_ctx.tff_atomic_formula):
            a_ctx = u_ctx.tff_atomic_formula()
            
            # Path 2a: Constant (e.g., world_5)
            if hasattr(a_ctx, 'tff_plain_atomic') and callable(a_ctx.tff_plain_atomic):
                p_ctx = a_ctx.tff_plain_atomic()
                if p_ctx and hasattr(p_ctx, 'constant') and callable(p_ctx.constant) and p_ctx.constant():
                    return p_ctx.constant().getText()

            # Path 2b: Defined Constant (e.g., $local_world)
            if hasattr(a_ctx, 'tff_defined_atomic') and callable(a_ctx.tff_defined_atomic):
                d_ctx = a_ctx.tff_defined_atomic()
                if d_ctx and hasattr(d_ctx, 'tff_defined_plain') and callable(d_ctx.tff_defined_plain):
                    p_ctx = d_ctx.tff_defined_plain()
                    if p_ctx and hasattr(p_ctx, 'defined_constant') and callable(p_ctx.defined_constant) and p_ctx.defined_constant():
                        return p_ctx.defined_constant().getText()
        
        # Fallback
        return u_ctx.getText()

    def get_unitary_term_constant_text(self, u_ctx) -> Optional[str]:
        """(NEW) Extract text of a plain constant from tff_unitary_term context."""
        if hasattr(u_ctx, 'tff_atomic_formula') and callable(u_ctx.tff_atomic_formula):
            a_ctx = u_ctx.tff_atomic_formula()
            if hasattr(a_ctx, 'tff_plain_atomic') and callable(a_ctx.tff_plain_atomic):
                p_ctx = a_ctx.tff_plain_atomic()
    ...
    def is_unitary_term_variable(self, u_ctx) -> bool:
        """(NEW) Check if a tff_unitary_term is a direct variable."""
        return hasattr(u_ctx, 'variable') and callable(u_ctx.variable) and u_ctx.variable() is not None

    def enterTff_defined_infix(self, ctx):
        """Catch equalities like $local_world = world_5 and harvest constants."""
        # Use correct grammar rule
        if not (hasattr(ctx, "defined_infix_pred") and ctx.defined_infix_pred()):
            return
        if ctx.defined_infix_pred().getText() != "=":
            return

        # FIX: The children are tff_unitary_term, not tff_term
        if hasattr(ctx, "tff_unitary_term"):
            lhs = ctx.tff_unitary_term(0) if ctx.tff_unitary_term(0) else None
            rhs = ctx.tff_unitary_term(1) if ctx.tff_unitary_term(1) else None
        else:
            lhs = rhs = None
        ...
        # FIX: Use the new helper functions for unitary terms
        lhs_txt = self.get_unitary_term_text(lhs)
        rhs_const = self.get_unitary_term_constant_text(rhs)
        lhs_const = self.get_unitary_term_constant_text(lhs)
        ...
        if self.is_unitary_term_variable(lhs) and rhs_const:
            v = self.get_unitary_term_text(lhs) # Use new helper
            if self.is_universally_quantified_world(ctx, v):
                self.worlds.add(rhs_const)
        if self.is_unitary_term_variable(rhs) and lhs_const:
            v = self.get_unitary_term_text(rhs) # Use new helper
            if self.is_universally_quantified_world(ctx, v):
    ...
    def get_term_text(self, term_ctx):
        ...
        # Path V (Variable)
        try:
            var_node = (term_ctx.tff_logic_formula().tff_unitary_formula().txf_unitary_formula().variable())
            if var_node:
        ...
        # Path C (Constant)
        try:
            const_node = (term_ctx.tff_logic_formula().tff_unitary_formula().tff_atomic_formula().tff_plain_atomic().constant())
            if const_node:
                return const_node.getText()
        except Exception:
            pass # Not a constant
            
        # --- NEW PATH D (Defined Constant) ---
        try:
            def_const_node = (term_ctx.tff_logic_formula().tff_unitary_formula().tff_atomic_formula().tff_defined_atomic().tff_defined_plain().defined_constant())
            if def_const_node:
                return def_const_node.getText()
        except Exception:
            pass # Not a defined constant
...
class TPTPModalParserANTLR:
    ...
    def build_interpretation_block(self, name: str, extractor: TPTPModalExtractor) -> str:
            ...
            if not body_parts:
                ...
                # Use "  " for 2-space indent, then 4 spaces for alignment
                body = "\n    & ".join(body_parts)
